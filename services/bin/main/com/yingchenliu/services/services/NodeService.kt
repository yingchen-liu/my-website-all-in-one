...
class NodeService(
    ...
    private fun sortChildrenAtCurrentLevel(node: TreeNode, orders: List<NodeOrder>): TreeNode {
        return node.children?.let { children ->
            ...
            return node.copy(children = (sortedOrderedNodes + unorderedNodes).map { node ->
                sortChildrenAtCurrentLevel(
                    node,
                    orders
                )
            })
    ...
    fun setup() {
        nodeRepository.save(
            TreeNode(
                uuid = UUID.fromString("b1747c9f-3818-4edd-b7c6-7384b2cb5e41"),
                name = "SkillTree",
                subtitle = null,
                content = null,
