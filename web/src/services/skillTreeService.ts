...
const processQueue = async () => {
  while (requestQueue.length > 0) {
    ...
    if (request) {
      const key = Object.keys(requestMap).find(
        (k) => requestMap[k] === request
      );
      try {
...
export const createChildNode = (node: TreeItem, parentUUID: string) => {
  queueRequest(node.uuid, "create", () => {
    return axios.post(`${API_BASE_URL}/${parentUUID}`, {
      clientId: clientId,
...
export const createNodeAfter = (node: TreeItem, previousNodeUUID: string) => {
  queueRequest(node.uuid, "create", () => {
    return axios.post(`${API_BASE_URL}/${previousNodeUUID}/after`, {
      clientId: clientId,
      operationId: operationId++,
      node,
    });
...
export const updateNode = (
  ...
  queueRequest(node.uuid, "update", () => {
    return axios.put(`${API_BASE_URL}/${node.uuid}`, {
      clientId: clientId,
      operationId: operationId++,
      node: modifiedNode,
    });
...
export const moveNode = (moveNodeDTO: MoveNodeDTO) => {
  queueRequest(moveNodeDTO.uuid, "move", () => {
    return axios.put(`${API_BASE_URL}/${moveNodeDTO.uuid}/position`, {
      clientId: clientId,
      operationId: operationId++,
      move: moveNodeDTO,
    });
